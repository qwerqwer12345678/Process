...
BOOL CTermpDlg::OnInitDialog()
{
	...
	m_list.InsertColumn(0, _T(" 장"), LVCFMT_CENTER, rect.Width() / 3);
	m_list.InsertColumn(1, _T("행"), LVCFMT_CENTER, rect.Width() / 3);
	m_list.InsertColumn(2, _T("번"), LVCFMT_CENTER, rect.Width() / 3);
...
void CTermpDlg::OnBnClickedButton1()
{
	...
	if (TF == TRUE)
	{
		...
		FileFind(pSCharInfo->m_char, loadtype);
...
//------------------------------------------초록네모-------------------------------------------
//------------------------------------------초록네모-------------------------------------------
void CTermpDlg::Grectangle(SCharInfo* pSCharInfo)
{
	CClientDC dc(this);
	CPen penG(PS_SOLID, 1, RGB(0, 255, 0));
	//CBrush brush(NULL_BRUSH);
...
//------------------------------------------파일찾기-----------------------------------------
//------------------------------------------파일찾기-----------------------------------------
void  CTermpDlg::FileFind(CString code, CString type)
{
	CFileFind finder;
	bool working = finder.FindFile(load1 + code + _T("\\*.*"));
	while (working)
	{
...
/*
		...
		if (finder.IsDirectory()) {
			...
			if (FindFile(finder.GetFilePath(), findname)) return true;
				...
				CString curfile = finder.GetFileName();     if (curfine == findfile)
